...
class Hangman():
    ...
    def is_invalid_letter(self, input_):
        ...
        """
        return (input_.isdigit()) or (not input_.isdigit() and len(input_) > 1)
    ...
    def update_progress(self, letter, indexes):
        ...
        """
        if len(indexes) == 1:
            self.game_progress[indexes[0][0]] = letter
        else:
            for i in range(len(indexes)):
                self.game_progress[indexes[i][0]] = letter
    ...
    def play(self):
        ...
        while True:
            ...
            if self.is_invalid_letter(user_input): # 플레이어가 입력한 인풋이 유효한 값인지 아닌지. 
                print('재대로 입력했나요?')# 아니면, 제대로 입력하라는 메세지 출력하고
                break# break or continue 중 하나를 사용하여 계속 입력 받음. 
            # Check if the letter is not already guessed
            if user_input in self.game_progress:
                print('이미 있는데 왜입력하지?')# 이미 입력받았던 알파벳이면, 이미 입력했다 메시지 출력
                break# break or continue 중 하나를 사용하여 계속 입력
